...
class _AppTreeState extends State<AppTree> {
  ...
  Widget maakTrackRij(int index, Track track) {
    return Dismissible(
      key: Key(track._title+track._artist+track._album),
  ...
  void play(int index, Track track) {
    if (mijnTimer!=null) mijnTimer.cancel();
    audioSpeler.play(track._file, isLocal: true);
    nrPlaying = index;
    mijnTimer = Timer.periodic(Duration(seconds: 1), (mijnTimer) {
